...
static const char *cgi_arg_get_value(const struct cgi_arg *this, const char *name)
{
	if (this == NULL) {
		return NULL;
	}
...
static const struct cgi_file *cgi_file_find(const struct cgi_file *this, const char *name)
{
	if (this == NULL) {
		return NULL;
	}
...
static void split_multipart(char *s, int length, const char *boundary, struct cgi_arg *arg, struct cgi_file *file)
{
	char *origin;
	int boundary_length;
	char *b;
	...
	for (;;) {
		...
		if (head - origin + strlen("Content-Disposition: form-data; name=\"") > length) {
			cgi_die("multipart format error");
		}
		if (strncmp(head, "Content-Disposition: form-data; name=\"", strlen("Content-Disposition: form-data; name=\"")) != 0) {
			cgi_die("multipart format error");
		}
...
static void read_post(struct cgi_input *input)
{
	...
	if (strcmp("application/x-www-form-urlencoded", content_type) == 0) {
		multipart = 0;
	} else if (strncmp("multipart/form-data;", content_type, strlen("multipart/form-data;")) == 0) {
		multipart = 1;
	} else {
	...
	*(input->post_data + length) = '\0';
	...
	if (!multipart) {
		...
		split_multipart(input->post_data, length, boundary, input->post, input->file);
	}
}
...
const char *cgi_input_get_value_compound(struct cgi_input *this, const char *name)
{
	const char *value;
...
const struct cgi_file *cgi_input_get_file(struct cgi_input *this, const char *name)
{
