...
static void attribute_init(void)
{
	attribute_depth = 0;
}
...
static int attribute_pop(void)
{
	...
	if (attribute_depth <= 0) cgi_die("attribute_stack underflow");
...
static const char *script_name; /* XXX should be in module's scope, initialized at main() or somewhere else. */
